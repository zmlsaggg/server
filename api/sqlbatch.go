...
// SAFE TRANSACTION
// ===============================
func SafeTransaction(engine *xorm.Engine, f func(*xorm.Session) error) (err error) {
...
// TRANSACTION (FIXED)
// ===============================
func (sb *SqlBank) transaction(session *xorm.Session) (err error) {
	if sb.banksum != 0 {
	...
	// FIX: InsertMulti -> Insert (xorm-safe batch insert)
	if len(sb.log) > 0 {
		if _, err = session.Insert(&sb.log); err != nil {
			return
		}
	}
...
// ===============================
// STORY TRANSACTION (FIXED)
// ===============================
func (ss *SqlStory) transaction(session *xorm.Session) (err error) {
	if len(ss.log) > 0 {
		_, err = session.Insert(&ss.log)
	}
	return
}
